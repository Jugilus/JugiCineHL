...
struct Collision
{
    ...
    static bool ShapesOverlap(VectorShape *shape1, VectorShape *shape2);

    // Returns true if the given **point** is inside **shape**; otherwise return false.
    static bool PointInShape(Vec2f point, VectorShape *shape);
    ...
    static bool RaycastToShape(Vec2f rayStart, Vec2f rayEnd, VectorShape *shape, Vec2f &intersection, float rayLength=-1);

    static bool RaycastToShapes(Vec2f rayStart, Vec2f rayEnd, std::vector<VectorShape*>&shapes, Vec2f &intersection, float rayLength=-1);
    static void GetRaycastToShapesIntersections(Vec2f rayStart, Vec2f rayEnd, VectorShape *shape, std::vector<PathPoint>&intersections, float rayLength=-1);
    ...
    static void TransformShapeToWorldCoordinates(VectorShape *srcShape, VectorShape *dstShape, ncine::Matrix4x4f m, Vec2f scale, float rotation);
...
private:
    static std::vector<PathPoint>intersections;
    ...
    static bool PolyToPoly(const std::vector<PathPoint> &poly1, const std::vector<PathPoint> &poly2);


    static bool CircleToPoly(Vec2f center, float radius, const std::vector<PathPoint> &poly);
    ...
    static bool PointInPoly(Vec2f point, const std::vector<PathPoint> &poly);


    //----- LINE to SHAPE

    static bool LineToPoly(Vec2f line_v1, Vec2f line_v2, const std::vector<PathPoint> &poly);


    static bool LineToCircle(Vec2f v1, Vec2f v2, Vec2f center, float radius);


    static void LineToPoly_GetIntersections(Vec2f line_v1, Vec2f line_v2, const std::vector<PathPoint> &poly, std::vector<PathPoint> &intersections);


    static void LineToCircle_GetIntersections(Vec2f line_v1, Vec2f line_v2, Vec2f center, float radius, std::vector<PathPoint> &intersections);
...
public:
    ...
    virtual void UpdateShapes(const ncine::Matrix4x4f &m) = 0;
    ...
    virtual bool Overlaps(VectorShape *_geomShape){ return false; }
...
public:
    ...
    VectorShape* GetShape(){ return shape; }

    virtual void AddShapes() override;
    virtual void UpdateShapes(const ncine::Matrix4x4f &m) override;
    ...
    virtual bool Overlaps(VectorShape *_geomShape) override;
...
public:
    ...
    std::vector<VectorShape*>& GetShapes(){ return shapes; }
