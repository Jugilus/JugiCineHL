...
#include "jugi/eFramework/map/eMap.h"
...
AnimationTrackState::AnimationTrackState(AnimationTrack* _animation, AnimationMemberState *_tAnimationsStatesSet)
{
    ...
    stateKey = AnimationKey::Create(animationTrack);
...
int AnimationTrackState::Update(int _animationTime, int _flags)
{
    ...
    if (kind==AnimationTrackKind::PATH_MOVEMENT){
        ...
        const std::string & pathNameID = animationTrack->GetTrackParameters().pathNameID;

        if(path && path->name()!=pathNameID){
            path = nullptr;
        }
        if(path==nullptr && pathNameID != ""){
#ifdef JUGI_EDITOR
            //path = CurrentMap->FindVectorShapeOfNameID(pathNameID);

            Map *map = nullptr;
            if(animationMemberState->sprite){
                map = animationMemberState->sprite->parentLayer()->map();

            }else if(animationMemberState->sprites){
                map = animationMemberState->sprites->front()->parentLayer()->map();
            }
            assert(map);

            //path = FindVectorShapeWithName(map, pathNameID);
            path = map->FindVectorShapeOfNameID(pathNameID);
...
int AnimationTrackState::ManageSubTimelineAnimation()
{
    ...
    if(k->animationName==""){
        ...
        returnFlag |= timelineAnimationPlayer->Play(&DummyAnimationInstance::dummy, flags);
...
int AnimationTrackState::ManageSubFrameAnimation()
{
    ...
    if(k->animationName==""){
        frameAnimationPlayer->Stop();
        int flags = ( k->discardAnimationsQueue)? AnimationPlayerFlags::DISCARD_ANIMATION_QUEUE : 0;
        returnFlag |= frameAnimationPlayer->Play(&DummyAnimationInstance::dummy, flags);
...
void AnimationTrackState::FindSourceObject()
{
    ...
    if(animationMemberState->sprite){
        sourceObject = animationMemberState->sprite->sourceSprite();

    }else if(animationMemberState->sprites){
        sourceObject = animationMemberState->sprites->front()->sourceSprite();
    }
...
Map* AnimationTrackState::FindMap()
{
    ...
    if(animationMemberState->sprite){
        return animationMemberState->sprite->parentLayer()->map();

    }else if(animationMemberState->sprites){
        return animationMemberState->sprites->front()->parentLayer()->map();
    }
	return nullptr;
}
...
TimelineAnimationInstance* AnimationTrackState::CreateAndStoreTimelineAnimationInstance(const std::string& _name)
{
    ...
    for(int i=0; i<sourceObject->timelineAnimations().size(); i++){
        if(sourceObject->timelineAnimations().at(i)->GetName() == _name){
            animation = sourceObject->timelineAnimations().at(i);
        }
    }
...
FrameAnimationInstance* AnimationTrackState::CreateAndStoreFrameAnimationInstance(const std::string& _name)
{
    ...
    for(int i=0; i<sourceObject->frameAnimations().size(); i++){
        if(sourceObject->frameAnimations().at(i)->GetName() == _name){
            animation = sourceObject->frameAnimations().at(i);
            break;
        }
    }
        ...
        */
        ...
        if(path){
            ...
            if(tp.pathDirectionOrientation){
                ...
                pathPoint = path->obtainPathPoint(p, directionAngle);
                ap.rotation -= directionAngle;

                ap.changeFlags |= Sprite::Property::ROTATION;

            }else{
                pathPoint = path->obtainPathPoint(p);
            }

            //---
            //Vec2f pathStartPoint(path->Points[0].x, path->Points[0].y);         //
            Vec2f pathStartPoint = path->pathPoints().front();
        ...
        if(sprite){
            sourceObject = sprite->sourceSprite();
        }else if(sprites){
            sourceObject = sprites->front()->sourceSprite();
        }

        AKFrameChange* k = static_cast<AKFrameChange*>(stateKey);

        if(k->frameImageIndex>=0 && k->frameImageIndex< sourceObject->sourceImages().size()){
            k->animationFrame.sourceImage = sourceObject->sourceImages()[k->frameImageIndex];
            ap.sourceImage = sourceObject->sourceImages()[k->frameImageIndex];
            ap.aniFrameOffset = k->animationFrame.offset;
...
TimelineAnimationInstance::TimelineAnimationInstance(TimelineAnimation *_timelineAnimation, Sprite *_sprite)
{
    ...
    assert(_timelineAnimation->GetSourceSprite()==_sprite->sourceSprite());
    ...
    if(animationsMemberStates.size()>1){      // child sprites
        ...
        assert(_sprite->type()==SpriteType::COMPOSITE);
...
//#ifdef JUGI_EDITOR
//        GatherSpritesWithSetNameID_B(_sprite->mSourceSprite->sourceCompositeSprite()->map(), childrenPerNameID);

//#else
       GatherSpritesWithSetNameID(static_cast<CompositeSprite*>(_sprite), childrenPerNameID);
...
//#endif
        ...
        for(int i=1; i<animationsMemberStates.size(); i++){
            ...
            for(std::vector<Sprite*>& childSprites : childrenPerNameID){
                ...
                if(childSprites.front()->name() == ams->animationMember->GetNameID()){
...
TimelineAnimationInstance::TimelineAnimationInstance(TimelineAnimation *_timelineAnimation, std::vector<Sprite*>&_sprites)
{
    ...
    for(Sprite *o : _sprites){
        assert(_timelineAnimation->GetSourceSprite()==o->sourceSprite());
    }
...
//#ifdef JUGI_EDITOR
        ...
        // TOLE DELUJE V EDITORJU KJER SO CHILD SPRAJTI ISTI ZA VSE MAP COMPOSED SPRAJTE IZ _sprites
        // V JUGIMAP API MORAM ZBRATI CHILD SPRITE ZA VSAK MAP COMPOSED SPRITE

//        GatherSpritesWithSetNameID_B(animation->GetSourceSprite()->sourceCompositeSprite()->map(), childrenPerNameID);

//#else
        for(Sprite* s : _sprites){
            assert(s->type()==SpriteType::COMPOSITE);
            GatherSpritesWithSetNameID(static_cast<CompositeSprite*>(s), childrenPerNameID);
        }
