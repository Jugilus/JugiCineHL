...
AnimationFrame::AnimationFrame(SourceImage *_sourceImage, int _durationMS) : sourceImage(_sourceImage)
{
    SetDuration(_durationMS);
}
...
void AnimatedProperties::Append(AnimatedProperties &ap)
{
    ...
    if((changeFlags & Sprite::Property::TEXTURE)==0){
        sourceImage = ap.sourceImage;
        aniFrameOffset = ap.aniFrameOffset;
...
DummyAnimationInstance DummyAnimationInstance::dummy;

//=============================================================================================================================


AnimationPlayer *AnimationPlayer::activeTimelineAnimationPlayer = nullptr;
...
int AnimationPlayer::Play(AnimationInstance *_animationInstance)
{
    ...
    if(_animationInstance==&DummyAnimationInstance::dummy){
    ...
    if(bp.loopForever==false && bp.repeat && bp.startAtRepeatTime){
        ...
        //print(" PLAY millisecondsPassed:" + std::to_string(msCurrentTime) + " msRepeat::" + std::to_string(msRepeat));
    ...
    latestUpdateReturnFlag = returnFlag;
...
int AnimationPlayer::Update()
{
    ...
    //print("AnimationPlayer::Update()  fSpeed:" + std::to_string(fSpeed)+"  msCurrentTime: " + std::to_string(msCurrentTime));
    ...
    if(state==AnimationPlayerState::PLAYING){
        ...
        if(msCurrentTime>=msRepeat){
            //print(" START millisecondsPassed:" + std::to_string(msCurrentTime) + " msRepeat::"+ std::to_string(msRepeat));
            //msStartPlayOffset = 0;
            countLoop = 0;
            msAnimationTimeStart = msCurrentTime;
            msCurrentAnimationTime = msStartPlayOffset;
...
int AnimationPlayer::GetRepeatTime(int _msCurrentTime)
{
    ...
    int msRepeatPeriod = bp.repeat_PeriodEnd-bp.repeat_PeriodStart;
    ...
    int msTime = _msCurrentTime + bp.repeat_PeriodStart + msRepeatPeriod * fRnd;
...
int AnimationQueuePlayer::PlayNextAnimationInQueue()
{
    ...
    */
    ...
    while(ai != animationInstances.back()){
        ...
        if(ai==&DummyAnimationInstance::dummy || ai->GetCompleteLoops()==false){
            bool removed = RemoveElementFromVector(animationInstances, ai);   assert(removed);
    ...
    if(ai==animationInstances.back()){
        if(ai==&DummyAnimationInstance::dummy){
            bool removed = RemoveElementFromVector(animationInstances, ai);   assert(removed);
            ai = nullptr;
        }else{
            returnFlag |= player.Play(ai);
