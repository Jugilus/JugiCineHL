...
//#ifdef JUGI_EDITOR
//#include "../jugi/eFramework/mapEVarious.h"
//#endif
...
struct AnimationFrame
{
    ...
    static const int durationMax = 10000;

    SourceImage *sourceImage = nullptr;              // LINK
    ...
    AnimationFrame(){}
    AnimationFrame(SourceImage *_sourceImage, int _durationMS=100);

    void SetDuration(int _durationMS){duration = (ClampValue(_durationMS,durationMin,durationMax)/10) * 10;}      // vrednosti na stotinko da se ujemajo z editorjem


    //editor
    float durationUnscaled = 100.0;
...
struct AnimatedProperties
{
    ...
    bool hidden = false;
    float alpha = 1.0f;
    ColorRGBA colorOverlayRGBA;
    ColorOverlayBlend colorOverlayBlend = ColorOverlayBlend::NORMAL;
    SourceImage *sourceImage = nullptr;
    Vec2f aniFrameOffset;

//#ifdef JUGI_EDITOR
//    TAnimationFrame *animationFrame = nullptr;
//    EShaderBasedProperties shaderBasedProperties;
//#endif
...
public:
    ...
    //----
    void Reset();
    void Add(EAnimatedProperties &ap);
...
struct AnimationBaseParameters
{
    ...
    int repeat_PeriodStart = 0;          ///< The repeat delay - start point time in milliseconds.
    int repeat_PeriodEnd = 0;            ///< The repeat delay - end point time in milliseconds. The actual repeat delay is a random value between the start and end point time.
...
/*
class EAnimationBaseParameters
{
public:
...
class DummyAnimationInstance : public AnimationInstance
{
public:
    static DummyAnimationInstance dummy;
...
public:
    ...
    static AnimationPlayer *activeTimelineAnimationPlayer;
    ...
    int GetLatestUpdateReturnFlag(){ return latestUpdateReturnFlag;}
