...
bool TransitionMember::init(PlayedScene *_scene)
{
...
bool Transition::init(PlayedScene *_scene)
{

    dbgSystem.addMessage("init transition '" + mName +"'");

    for(TransitionMember* tm : mObjects){
        if(tm->init(_scene)==false){
            return false;
        }
    }
...
bool TransitionsGroup::init(PlayedScene *_scene)
{
