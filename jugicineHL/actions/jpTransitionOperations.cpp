...
bool TranslateInOutOfScreen::init(PlayedScene *_scene)
{

    std::vector<void*> &objects = mParentObject->objects();
    GfxObjectType objectTypeId = mParentObject->objectType();


    if(mTransitionType==TransitionType::OUT){
        dbgSystem.addMessage("init '" + typeOut + "'");

    }else if(mTransitionType==TransitionType::IN){
        dbgSystem.addMessage("init '" + typeIn + "'");
    }
...
void TranslateInOutOfScreen::onTransitionStarted()
{
    ...
    std::vector<void*> &mObjects = mParentObject->objects();
    GfxObjectType mObjectType = mParentObject->objectType();
...
bool AlphaFadeInOut::init(PlayedScene *_scene)
{
    ...
    if(mTransitionType==TransitionType::IN){
       ...
       // mTween.Init(mStartAlpha, 0.0f, mDurationS, jugimap::EasingKind::EASE_END_QUAD);
        ...
        if(mAlphaMin>0.01){
            mParentObject->_setSkipOnTransitionFinished(true);
        }
    }
...
bool ScaleInOut::init(PlayedScene *_scene)
{
