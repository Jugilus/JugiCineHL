...
bool GuiTable::init()
{
    ...
    if(layout=="horizontal"){
        mLayout = Layout::HORIZONTAL;
    }else{
        mLayout = Layout::VERTICAL;
    }
    ...
    if(sprites.empty()){
        dbgSystem.addMessage("Missing sprite with 'wFirstSlot' parameter in table '" + name + "'!");
        return false;
    }
    ...
    if(sprites.empty()==false){
        ...
        if(sprites.front()->type()!=SpriteType::COMPOSITE){
            dbgSystem.addMessage("Sprite for slider of the table'" + name + "' must be a composite sprite!");
            return false;
        }
    ...
    */
    ...
    if(mSelectedItem){
        ...
        if(mSelectedItem->doOnSelected()){
            mSelectedItem->doOnSelected()->update(UpdateMode::NORMAL);
        }
    }
    ...
    if(mSelectedItem){
        print("Table '" + name + "' selected item '" +mSelectedItem->name() +"'.");
    }else{
        print("Table '" + name + "' no selected item.");
    }
...
void GuiTable::SetToInitialState()
{
    ...
    for(GuiMultiSlot *ms : mMultiSlots){
        for(GuiSlot *s : ms->mSlots){
            s->selectingButton()->setInitialState(GuiButtonVisualState::DISABLED);
...
void GuiTable::updateSlots()
{
    ...
    if(mSlider){
        ...
        if(mKind==Kind::FIXED_SLOTS){
            updateSlotsItems_FixedSlots();

        }else if (mKind==Kind::SCROLLING_SLOTS){
...
bool GuiSlot::init()
{
    ...
    for(Sprite *s : sprites){
        ...
        /*
        const std::string & icSpriteMarkerName = s->parameters().value("icTextSprite");
        if(icSpriteMarkerName.empty()){
            dbgSystem.addMessage("Sprite with source sprite '" + s->sourceSprite()->name() +"' is missing value of parameter 'icTextSprite'!");
            return false;
        }

        if(s->type()!=SpriteType::TEXT){
            dbgSystem.addMessage("Sprite with source sprite '" + s->sourceSprite()->name() +"' has defined 'icTextSprite' parameter but is not a text sprite!");
            return false;
        }
        */
...
void GuiSlot::addItem(GameItem *_item)
{
    ...
    if(mGameItem->isSelectable()){
        ...
        if(mParentMultislot->parentWidget()->GetKind() == GuiWidgetKind::TABLE){
...
void SelectingSlotCallback::OnPressed(jugimap::GuiWidget *_widget)
{

    GuiButton *b = static_cast<GuiButton*>(_widget);

    if(mSlot->parentMultislot()->parentWidget()->GetKind()==GuiWidgetKind::TABLE){
